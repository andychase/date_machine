...
(function() {
  ...
  fillResult = function(_) {
    var key, result, resultSet, _j, _k, _len1, _len2;
    for (_j = 0, _len1 = _.length; _j < _len1; _j++) {
      resultSet = _[_j];
      for (_k = 0, _len2 = resultSet.length; _k < _len2; _k++) {
        result = resultSet[_k];
        for (key in result) {
          if (result[key] == null) {
            delete result[key];
  ...
  dateTest = function() {
    var answerObject, testString;
    testString = arguments[0], answerObject = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    return deepEqual(fillResult(_this.dateMachine(testString)), [answerObject], testString);
  };

  dateRawTest = function(testString, answerObject) {
    return deepEqual(fillResult(_this.dateMachine(testString)), answerObject, testString);
  };
  ...
  test("date tests", function() {
    ...
    return dateTest('Sun, 06/16/2013 - 6:00pm', {
      hour: 18,
      month: 6,
      year: 2013,
      day: 16,
